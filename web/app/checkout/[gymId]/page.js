...
function CheckoutPageInner() {
  ...
  if (pass.price <= 0) {
    return (
      <div className="max-w-md mx-auto px-6 py-20 text-center">
        ...
        <Link href={`/gyms/${gym.id}`} className="text-brand-text dark:text-blue-400 hover:underline">← Back to {gym.gymName}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-md mx-auto px-6 py-12">
      <Link href={`/gyms/${gym.id}`} className="text-brand-text dark:text-blue-400 hover:underline text-sm font-semibold">
        ← Back to {gym.gymName}
      </Link>
