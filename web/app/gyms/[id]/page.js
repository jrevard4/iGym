...
export default function GymDetailPage() {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-20 text-center text-gray-400">Loading gym...</div>
    );
  }
...
function PassCard({ pass, gym }) {
  ...
  const onBuy = () => {
    ...
    // For v0: announce and link out to the mobile app. Real Stripe Checkout is
    // documented in web/README.md as the next step.
    setBuying(true);
    alert(
      `Pass checkout coming next to the website. For now, open the iGym mobile app and tap "${pass.label}" at ${gym.gymName} to complete purchase.`
    );
    setBuying(false);
  };
