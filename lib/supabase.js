...
// Confirmed bookings platform-wide that haven't had a "starts soon" reminder
// push sent yet — the source list for /api/send-class-reminders. Waitlisted
// bookings are excluded: they don't hold a confirmed seat, so a reminder
// would be misleading.
export async function loadBookingsNeedingReminder() {
  if (!supabase) return [];
  const { data, error } = await supabase.from('classBookings').select('*')
    .eq('status', 'booked').eq('reminderSent', false);
  warn('loadBookingsNeedingReminder', error);
  return data || [];
}

export async function markReminderSent(bookingId) {
  if (!supabase) return;
  const { error } = await supabase.from('classBookings').update({ reminderSent: true }).eq('id', bookingId);
  warn('markReminderSent', error);
}
