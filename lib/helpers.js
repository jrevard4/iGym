...
// Resolves whether a page section should render for a given gym: an
// explicit true/false in gym.pageSettings wins, otherwise falls back to
// that section's default (see PAGE_SECTIONS) — so a gym with no
// pageSettings on file (every gym before this feature existed) renders
// exactly as it did before.
export function isSectionVisible(gym, key) {
  const explicit = gym?.pageSettings?.[key];
  if (explicit === true || explicit === false) return explicit;
  return PAGE_SECTIONS.find((s) => s.key === key)?.default ?? true;
}
...
// Expands a gym's recurring weekly class template (gym.classSchedule) into
// concrete bookable dates over the next `weeksAhead` weeks. Each schedule
// entry is a {id, className, dayOfWeek (0=Sun), startTime, durationMinutes,
// capacity, instructor} template — this is what turns "every Monday 6am
// Yoga" into an actual list of upcoming calendar occurrences to book.
// `now` is injectable for tests; defaults to the real current time.
export function getUpcomingClassOccurrences(gym, weeksAhead = 2, now = new Date()) {
  const schedule = gym?.classSchedule || [];
  if (schedule.length === 0) return [];
  const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const occurrences = [];

  for (let dayOffset = 0; dayOffset < weeksAhead * 7; dayOffset++) {
    const date = new Date(startOfToday.getTime() + dayOffset * 86400000);
    const dow = date.getDay();
    schedule.forEach((cls) => {
      if (Number(cls.dayOfWeek) === dow) {
        occurrences.push({ ...cls, classDate: date.toISOString().slice(0, 10) });
      }
    });
  }

  return occurrences.sort((a, b) => (a.classDate + a.startTime).localeCompare(b.classDate + b.startTime));
}

// Counts active ('booked', not 'waitlisted'/'cancelled') reservations for one
// specific occurrence of a recurring class — used to enforce capacity and
// decide whether a new reservation should be waitlisted instead.
export function countBookedForOccurrence(bookings = [], classScheduleId, classDate) {
  return bookings.filter(
    (b) => b.classScheduleId === classScheduleId && b.classDate === classDate && b.status === 'booked'
  ).length;
}

// Platform-wide KPIs for the admin dashboard — pure aggregation over data
// already loaded elsewhere (loadGyms/loadUsers), no separate DB queries.
export function computePlatformStats(gyms = [], users = []) {
  const totalRevenue = gyms.reduce((sum, g) => sum + (Number(g.totalPassRevenue) || 0), 0);
  const totalPlatformFees = gyms.reduce((sum, g) => sum + (Number(g.platformFeesPaid) || 0), 0);
  const totalReviews = gyms.reduce((sum, g) => sum + (g.gymReviews?.length || 0), 0);
  const topGymsByRevenue = [...gyms]
    .sort((a, b) => (Number(b.totalPassRevenue) || 0) - (Number(a.totalPassRevenue) || 0))
    .slice(0, 10);
...
// Aggregates a gym's recurring-membership passes (see loadGymPasses in
// lib/supabase.js — every pass ever sold, not filtered) into MRR and
// churn-risk numbers for the owner analytics dashboard. Only passes with a
// stripeSubscriptionId are recurring; a one-time TIME/PUNCH pass never
// counts here even if it happens to still be within its validity window.
// Each subscription's billing period (`value`, in days) is normalized to a
// 30-day month so a weekly and a monthly membership contribute comparably.
export function computeRecurringRevenueStats(passes = []) {
  const subscriptions = passes.filter((p) => p.stripeSubscriptionId);
  const active = subscriptions.filter((p) => p.status === 'active' || !p.status);
  const pastDue = subscriptions.filter((p) => p.status === 'past_due');
  const canceled = subscriptions.filter((p) => p.status === 'canceled');

  const mrr = active.reduce((sum, p) => {
    const billingDays = parseInt(p.value) || 30;
    return sum + (Number(p.price) || 0) * (30 / billingDays);
  }, 0);
